...
| レイテンシはどう違うか | 検索区間の中央値で **OpenSearch 213 ms ÷ DynamoDB 19.5 ms = 10.9 倍**。Cold_Start は DynamoDB 398 ms 対 OpenSearch 18,356 ms（`took`）で 2 桁違う（実測）。**ただしエンドツーエンドでは埋め込み生成 203 ms が約 3 分の 2 を占めるため実務上の選定軸にならない**（第 5.1 節） |
...
| **DynamoDB Vector Search は実務で使えるか** | **特定の形のワークロードには使える。その形は宣伝が想起させるより狭い。**採否を決めるのはレイテンシでも recall でもなく、**範囲フィルタの要否・100 件上限・既存テーブルの GSI 射影・検索の権限分離**の 4 点である。詳細は第 1.1〜1.7 節 |

### 1.1 総合判断

**レイテンシと recall では採否が決まらない。** どちらも両バックエンドで実務上問題のない水準に収まり、しかもエンドツーエンドでは差が体感に出ない（第 5.1 節）。判断を決めるのは**機能制約と、ベクトルを載せる既存テーブルの形**である。

以下は本検証の実測に限定した判断であり、測定していない事項は第 1.7 節に分けて記す。

### 1.2 採否を決める 4 つの質問

**質問 1: 範囲フィルタが必要か。** これが最大の分岐点である。**等価条件のみで、比較・範囲・`BETWEEN`・`IN` はいずれも HTTP 400 で拒否される**（実測。第 7.1 節）。在庫・商品検索で頻出する「価格 3,000 円以下」「在庫あり」「今月入荷分」を 1 回の呼び出しで表現できない。

回避策は TopK 100 で取得してからアプリケーション側で絞ることだが、**絞り込み後の残存件数が保証されない。** 条件が狭ければ 100 件取得して数件しか残らず、「もっと見る」に相当する操作が原理的に提供できない。**ここが要件に含まれるなら、OpenSearch の併用か別の索引が必要になる。**

**質問 2: 上位 100 件で足りるか。** TopK 上限が 100 で、**応答にページネーションがない**（16 MB 上限だがそれ以前に API 仕様の 100 件で切れる。第 3 節のアイテムサイズ分析）。101 件目以降は取得できない。「20 件ずつ無限スクロール」のような UX は構築できない。

**質問 3: 既存テーブルの GSI が `ProjectionType: ALL` か。** 見落とされやすく、かつ影響が大きい。本検証の対象テーブルは GSI 3 本すべてが `ALL` であり、**ベクトルを追加すると全 GSI に複製されて 12,297,304 B（約 12.3 MB）から約 512 MB へ、約 42 倍に膨張する**（基底は実測、追加後は推定。知見 1）。既存の在庫一覧 Query は 1 ページ約 5 KB から約 150 KB へ（RCU 約 1 → 約 19）増え、**既存の測定値がすべて無効化される。**

対策は GSI の射影を `KEYS_ONLY` または `INCLUDE` に変更することだが、既存クエリが射影外の属性を参照していないことを全件確認しなければ変更できない。**本番テーブルに対しては軽い変更ではない。**

**質問 4: 検索に行レベルのアクセス制御が必要か。** **`SearchVectors` にはファイングレインアクセスコントロールの条件キーが効かない**（`dynamodb:LeadingKeys` / `dynamodb:Attributes` / `dynamodb:Select` はいずれも無効。第 12 節）。したがって**パーティションキーによるテナント分離は検索に対して機能しない。**「東京倉庫の担当者は東京の在庫のみ検索できる」を IAM で表現できない。分離できる粒度はインデックス単位のみで、テーブルあたり上限 5 本である。

### 1.3 DynamoDB が明確に勝つ点

**アイドル復帰の速さ。これが本検証で最も実用上の差が出た項目である。**

OpenSearch Serverless の scale-to-zero は実際に機能し**アイドル時課金 0 USD** になるが、代償として**アイドル後の初回検索が 18〜19 秒かかる**（`took` 19,349 ms と 18,356 ms を 2 回独立に観測。実測。第 4.4 節）。対する DynamoDB は Lambda の初期化を含めて 400 ms 程度である（`searchLatencyMs` 398 ms / `handlerLatencyMs` 687 ms。実測。第 3.1 節）。

**1 日に数回しか使わない社内ツールでは、この差が決定的である。** 18 秒待つ検索画面は実用に耐えない。OpenSearch でこれを避けるには最小 OCU を 1 以上に設定する必要があり、その場合**月約 175 USD が常時発生する**（1 OCU × 0.24 USD × 730 h。推定）。**「アイドルコストを 0 にする」と「初回応答を速くする」を同時に満たせないのが OpenSearch 側の構造的な弱点であり、DynamoDB にはこのトレードオフが存在しない。**

**インフラの面積が小さい。** Collection、Collection Group、暗号化・ネットワーク・データアクセスの 3 ポリシー、OCU のサイジング、これらがすべて不要になる。既存テーブルにインデックスを 1 本追加するだけである。

**ストレージ費用は意思決定の要因にならない。** PoC 規模で月約 0.07 USD、本番想定 150,000 レコードでも月約 0.72 USD（推定。第 10 節）。次元数を削減する動機はストレージ費用ではない。

**本来の主要な価値は同居である。** ベクトルと業務データが同一テーブルに存在し、1 回の `SearchVectors` で商品名・在庫数・ロケーション・単価まで取得できる（本検証でも射影した 6 属性が返ることを確認済み）。OpenSearch では「検索してから DynamoDB を引き直す」2 段構えになる。**ただし本検証は測定の分離を優先して専用テーブルを新設したため、この価値そのものは実測していない**（知見 2）。

### 1.4 成熟度（GA から 2 週間強の時点での実測）

DynamoDB Vector Search の GA は 2026-08-05 で、本検証の測定は 2026-08-18〜22 に実施した。**GA から 2 週間強の段階**であり、以下はその時点の観測である。

| 事象 | 内容 | 参照 |
|---|---|---|
| CloudFormation が表現できない | `AWS::DynamoDB::Table` に `VectorIndexes` プロパティが存在せず、カスタムリソースを自作した | V1 |
| SDK モデルの遅れで情報が黙って消える | `TableDescription.VectorIndexes` が返るのは **3.1103.0 以降**。それ以前では**エラーも警告もなく欠落**し、ACTIVE なインデックスに対して `INDEX_NOT_FOUND` という別の症状に化けた。AWS CLI 2.35.9 でも欠落する | 知見 10 |
| 消費キャパシティ項目がモデルにない | `VectorSearchUnits` が SDK の `VectorCapacity` に存在せず、生応答から読む必要がある | 知見 8 |
| 公式ドキュメントの矛盾 | 範囲フィルタの可否が開発者ガイドと SDK API リファレンスで食い違っていた（正しいのは開発者ガイド） | 第 7.1 節 |
| バックフィル完了が観測できない | `Backfilling` の `true → false` 遷移を一度も観測できず、完了までの経過時間が測定不能 | 知見 9 |
| リクエスト形式の非自明さ | `SearchVector` は `AttributeValue[]`（`[{"N":"..."}]`）でのみ受理され、素の数値配列は `SerializationException` | 知見 8 |
| インデックス作成の所要時間 | 1 本あたり ACTIVE 到達まで **546 秒 / 542 秒**、2 本を逐次で約 18 分（実測） | 第 7.3 節 |

**いずれも回避できたが、それぞれ数時間を要した。** 新しいサービスに特有の状態であり、時間の経過で改善が見込まれる種類の問題である。ただし現時点で採用するなら、この工数を見込む必要がある。

### 1.5 データモデル起因の減点をしないこと

本検証で目立った「3 行複製により TopK が 3 分の 1 に希釈される」「一意 SKU が 33 件しか取得できない」「兄弟の回収漏れ」は、**DynamoDB Vector Search の制約ではなく本検証のデータモデルの都合である。** 同一 SKU が 3 倉庫分の行を持ち、その 3 行が同一ベクトルを保持しているために生じる（A11、知見 3）。

**本番設計でベクトルを SKU 単位の 1 行に配置すれば消える。** その場合 TopK 100 がそのまま 100 件の一意 SKU に対応し、兄弟の回収漏れという問題自体が発生しない。**DynamoDB を評価する際にこれを減点材料としてはならない。**

### 1.6 採否チェックリスト

次のすべてに当てはまる場合、DynamoDB Vector Search は積極的に選べる。**OpenSearch という 1 システムをアーキテクチャから消せるため、運用上の利得が大きい。**

- 既存の DynamoDB テーブルに意味検索を追加したい
- 絞り込みは等価条件で足りる（カテゴリ、倉庫、ステータスなど）
- 上位 100 件で足りる（ページネーション不要）
- 対象テーブルの GSI が `ProjectionType: ALL` でない、または射影を変更できる
- 検索の権限分離が不要か、インデックス単位（上限 5 本）で足りる
- 利用が散発的で、アイドル後の初回応答の速さが重要である
- ベクトルを 1 エンティティ 1 行で配置できる

**いずれか 1 つでも外れる場合は、OpenSearch の併用か別の設計を検討したほうが早い。** とくに範囲フィルタは後から追加できない（距離関数と同様にインデックス作成時に決まる性質ではないが、サービス側が対応していないため回避策しかない）。

### 1.7 本検証では答えられていないこと

**総額の比較ができていない。** `VectorSearchUnits` の単価を確認していないため、DynamoDB 側の検索費用を金額に換算できない。1 検索あたり **61,307〜117,244 バイト**（TopK 1〜100 に依存。実測。第 3.5 節）という数量は確定したが、単価が不明なため月 300,000 クエリの費用が出せない。OpenSearch 側は月 193〜350 USD と見積もれる（推定）ので、**DynamoDB の単価が判明して初めて総額比較が成立する。** 選定の最終判断にはこれが必要である。

**同居の価値を実測していない。** 第 1.3 節に述べたとおり、本検証は専用テーブル方式を採ったため「1 回の呼び出しで業務属性まで取得できる」ことの定量的な利得（OpenSearch の 2 段構えとの往復回数・レイテンシ・コストの差）を測っていない。**これは DynamoDB Vector Search の主要な価値であり、本検証の最大の測定漏れである。**

**運用時の信頼性判定が要件に含まれていない。** Ground_Truth を持たない本番環境では recall を計算できない。代替としてカテゴリ整合性と兄弟の完全性を見る案を本検証で得たが（知見 11）、これは要件として定義されていなかった。ベクトル検索を本番に載せる場合、設計時に決めておく必要がある。
...
| 倉庫フィルタ | recall 測定・レイテンシ測定・キャパシティ測定のいずれも**適用なし（全倉庫）**。等価フィルタの受理は Q1 プローブの対照ケース（`warehouseId = WH-TOKYO`）と、撤収前に実施した UI からの実測（第 9.6 節）で確認した |
...
**上表の数値には測定条件の限定が付く。** 本節の 30 試行は Paired_Query_Set 5 件 × 2 言語 × 3 回で、**クエリベクトルが毎回変わる。** ブラウザ UI から同一クエリを 10 回反復すると値が大きく下がる（いずれも `searchLatencyMs`、実測）。

| 条件 | DynamoDB | OpenSearch |
|---|---|---|
| 同一クエリを 10 回反復（10 回目） | **23 ms** | **99 ms** |
| クエリを変えながら（暖機後 14 回の範囲） | 71〜481 ms | 289〜1,046 ms |

同一クエリの反復では DynamoDB が本節の中央値 19 ms に収束し、OpenSearch は本節の実測最小 116 ms を下回る 99 ms に達した。**同一の探索経路とキャッシュを繰り返し叩くため楽な側に振れる。**逆にクエリを変える条件では DynamoDB が本節の最大 240 ms を超える（481 ms）。**数値を引用する際は必ず条件を併記すること。**

**アイドル後の初回には接続確立コストが乗る。** ブラウザ経路で同一クエリを繰り返すと 301 → 140 → 23 ms と単調に落ちた（実測）。本節の中央値 19 ms は**連続負荷下の値**であり、1 日に数回しか検索しない用途の体感値ではない。
...
すなわち **近似探索が、同一ベクトルを持つ 3 つの倉庫兄弟をすべて回収しきれず、1〜2 行しか返さない SKU が生じる。** その結果 99 行という窓に余分な SKU が入り込み、本来上位にあるべき SKU が窓の外へ押し出される。これは近似最近傍探索が同値の重複ベクトルに対して持つ性質であり、本実装の欠陥ではない。

**ただしこれは OpenSearch 固有の性質ではない。** 上表だけを見ると「DynamoDB は完全、OpenSearch は近似」と読めるが、**それは q43 に限った像である。** ブラウザ UI から TopK 99 で 9 クエリを実測したところ（`docs/measurements/ui-investigation-2026-08-22.json`）、**どちらが劣るかはクエリによって入れ替わる**（いずれも日本語 / 全倉庫 / 各 1 回、実測）。

指標は `返却行数 99 ÷ 一意 SKU 件数` で、3.00 なら三つ組が完全に揃っている。

| クエリ | 1 位距離 | DynamoDB | OpenSearch |
|---|---|---|---|
| q02 `チョコレートのような甘さの深煎りブレンド` | 0.5063 | **3.00** | **3.00** |
| q43 `ハンドドリップに向いた焙煎豆` | 0.5195 | **3.00** | 2.36 |
| q47 `1kg の業務用の大容量焙煎豆` | 0.6915 | **3.00** | 2.61 |
| q48 `200g の小容量パックの焙煎豆` | 0.7276 | 2.48 | 2.41 |
| **q50 `アイスコーヒー向けの深煎り豆`** | 0.7566 | **1.90** | **2.91** |
| `業務用の大容量焙煎豆`（即席） | 0.7629 | 2.68 | 2.68 |

**q50 で立場が完全に逆転する。** DynamoDB は 1 SKU あたり 1.90 行しか返さず（順位 1・2・3 が別々の SKU）、OpenSearch は 2.91 でほぼ完全な三つ組を返す。したがって **DynamoDB Vector Search も近似索引であり、兄弟を取りこぼす。** 「DynamoDB は Ground_Truth を完全に再現する」は q43 での観測であって一般的な性質ではない。

傾向としては、**DynamoDB は 1 位距離 0.72 付近まで 3.00 を保ち、そこから崩れる。OpenSearch は距離に関係なく崩れる**（q43 は距離 0.5195 で 2.36）。近似の質はこの点で異なるが、優劣は一方向ではない。

**q50 は説明が付いていない。** サイズ語を含まないクエリであるのに DynamoDB が特異に崩れる理由は未解明であり、単独の未解決事項として記録する。

**含意は本番設計に効く。** 同一ベクトルを N 行に複製する構成は、TopK を N 分の 1 に希釈するだけでなく（知見 3）、**どの近似索引でも回収漏れを生む。** ベクトルを SKU 単位の 1 行に置けばこの劣化要因は消える。
...
**そして検索区間の比較は実務上の選定軸にならない。** 完全に暖まった定常状態をブラウザ UI で実測すると、内訳は次のとおりである（実測）。

| 区間 | 時間 | 全体に占める割合 |
|---|---|---|
| 埋め込み生成 | 203 ms | **67%** |
| 検索（DynamoDB） | 23 ms | 8% |
| 検索（OpenSearch） | 99 ms | 33% |

利用者が体感するのは「埋め込み 1 回 + 検索（並列なので遅い側）」なので **203 + 99 ≈ 302 ms** であり、**埋め込み生成が約 3 分の 2 を占める。** DynamoDB を選ぶことで節約できるのは 302 ms のうち 76 ms（約 25%）にとどまり、**検索区間の中央値比 10.9 倍が与える印象と実際の効き方は乖離する。**

**埋め込み生成は暖機で改善しない。** 同一クエリを 10 回反復しても 191 → 202 → 203 ms で、検索側が大幅に落ちるのとは対照的である（実測）。us-west-2 の `amazon.titan-embed-text-v2:0` はレイテンシ最適化推論に非対応であり、毎回 `ValidationException` を受けてから標準推論で呼び直すため **Bedrock への往復が 2 回発生する**（知見 7、要件 18.22）。この失敗する往復が 203 ms のうち何 ms を占めるかは**未実測**である。分離するには Bedrock を直接 2 回叩いて `performanceConfigLatency` の有無で所要時間を比べればよい。

**両バックエンドがエンドツーエンドで同オーダーに収まるため、レイテンシは選定の判断材料にならない**（検証担当者の判断）。差が出るのは recall・機能制約・コストの側である。
...
**この例は当初「日英でほぼ同一の挙動」と記述していたが、商品 ID を確認すると誤りであった。** どちらも「袋 × バルブ付」に正確に当たっている点は正しいが、**引き当てている SKU 群が違う。**

| 言語 | 上位 3 SKU の itemId |
|---|---|
| ja | `MAT-BAG-1KG-VALVE` / `MAT-BAG-100G-VALVE-V1128` / `MAT-BAG-200G-VALVE` |
| en | `MAT-BAG-L-VALVE` / `MAT-BAG-M-VALVE` / `MAT-BAG-S-VALVE` |

**日本語は容量表記（1kg用 / 100g用 / 200g用）の袋、英語はサイズ表記（L / M / S）の袋で、重なりは 0 である。** 各 SKU は日英両方の名前を持つのに、クエリ言語がその言語の命名規則を持つ SKU 群を引き当てている。したがって q53 は「言語差が出ない対照例」ではなく、**3 例の中で最も明快に語彙の分岐が現れた例**である（ブラウザ UI からの実測。`docs/measurements/ui-investigation-2026-08-22.json`）。
...
| 項目 | 値 | ラベル |
|---|---|---|
| 対象クエリ | `intent === 'flavor'` の **20 件**（q01〜q20。全文は第 2.4 節） | 定義 |
| 集計単位 | バックエンド 2 × 言語 2 × Distinct_Sku_K 3 = **12 集計** | 定義 |
...
#### 適用範囲の限定（この保証は flavor 群にしか及ばない）

**検査したのは `intent === 'flavor'` の 20 件のみである。** 他の intent は負例クラスの検査対象に含めていない。ブラウザ UI から usage 群を TopK 99 で実測したところ、**資材が上位を占めるクエリが存在した**（日本語 / 全倉庫 / 各 1 回、実測）。

| クエリ | 1 位距離 | 資材が現れた最初の順位 | 上位の内容 |
|---|---|---|---|
| `1kg の業務用の大容量焙煎豆` | 0.6915 | **1 位** | 資材 ラベル 1kg用（クリア / バルブ付 / アルミ）が順位 1〜9 を占め、実際の豆は順位 10 から |
| `200g の小容量パックの焙煎豆` | 0.7276 | **4 位** | グアテマラ焙煎豆と資材 袋 200g用が混在 |
| `業務用の大容量焙煎豆`（即席） | 0.7629 | **1 位** | 資材 袋 XL（バルブ付 / クリア / アルミ） |

**焙煎豆を求めるクエリの上位 9 行が包装資材である**という状態が実在する。したがって「意味検索がカテゴリ境界を捉えている」という結論は **flavor 群に限定される。**

**原因はサイズ語の存在そのものではない。** サイズ語を含みながら資材が出ないクエリがある。

| クエリ | サイズ語 | 該当商品 | 1 位距離 | 資材 |
|---|---|---|---|---|
| `500g の深煎りブレンド` | あり | あり | 0.4926 | **0 件** |
| `1kg の深煎りブレンド` | あり | **なし**（ブレンドは 500g まで） | 0.5855 | **0 件**（500g のブレンドへ落ちた） |
| `カフェインレスの豆` | **なし** | なし（属性が存在しない） | 0.7103 | **0 件**（ケニア生豆） |

**問題はサイズ語以外に照合できる属性がないことである。** Sku_Metadata の 9 項目に照らすと差が説明できる。`500g の深煎りブレンド` は「深煎り」が焙煎度、「ブレンド」がカテゴリに一致するため、サイズ語より強い信号が存在する。一方 `1kg の業務用の大容量焙煎豆` は「業務用」「大容量」がどの項目にも一致せず、「1kg」が商品名にしか当たらないため、**サイズを第一の記述子とする資材が勝つ**（資材の商品名は `資材 袋 XL バルブ付` `資材 ラベル 1kg用 クリア` のようにサイズが主要な識別要素である）。

**距離が遠いだけでは資材は混入しない。** `カフェインレスの豆` は距離 0.7103 と遠く該当属性も存在しないが、資材は 1 件も現れずケニアの生豆に留まった。詳細は知見 11 に記す。
...
### 9.6 倉庫フィルタの実測（要件 8.6 / 13.14 / Property 45）

**recall・レイテンシ・キャパシティの各測定はいずれも倉庫フィルタなしで実施した。** そのためフィルタ有効時の挙動は撤収直前に UI から 1 回だけ実測した（日本語 / TopK 30 / `warehouseId = WH-TOKYO`。クエリは第 9.3 節の q22）。

| | 全倉庫 | WH-TOKYO |
|---|---|---|
| 返却行数 | 30 | 30 |
| **一意 SKU 件数** | **10** | **30** |
| 返却行数 ÷ 一意 SKU | 3.00 | **1.00** |
| 両バックエンドの一致 | 一意 SKU 10 件 / 順位差 0〜2 | **一意 SKU 30 件 / 並びが完全一致** |

**これは知見 3 の希釈が倉庫行数ちょうどであることの直接の証拠である。** 等価フィルタで各 SKU が 1 行に絞られ、同一の TopK 30 で 3 倍の一意 SKU に到達した。返却行は全件 `WH-TOKYO` であり、要件 8.6 の等価フィルタが実環境で機能している。

**距離の完全同値が消え、両バックエンドの並びが完全に一致した。** 上位 10 件の距離は 0.5356 / 0.5418 / 0.5427 / 0.5430 / 0.5438 / 0.5440 / 0.5457 / 0.5459 / 0.5470 / 0.5471 ですべて異なる。全倉庫では三つ組内の倉庫順が入れ替わって順位差 0〜2 が生じていたが、1 SKU 1 行になると順序が一意に決まる。**近似探索であっても同値がなければ順序は一致する**ことの実例であり、13.17 の同値分析（ε = 1e-6、境界同値 0/60）が必要だった理由がここに現れている。

**Property 45 の「十分大きな TopK において部分集合」という条件付けが必要であることも確認できた。** 上位 3 SKU は全倉庫版と同一だが、フィルタ版は 4 位以降で全倉庫版が到達していなかった SKU へ進む。**同一の TopK では部分集合にならない。**

### 9.7 入力検証の実機確認（要件 11.6 / 11.9 / Property 30）

第 10 節の UI 実装は jsdom のコンポーネントテストで検証しているが、実環境での確認は撤収直前に 1 回だけ行った。TopK に `1.5` を入力して検索ボタンを押した結果は次のとおりである（実測）。

| 確認項目 | 結果 |
|---|---|
| 検索を実行しない | **OK**（レイテンシ表示が 258 / 791 ms のまま変化なし） |
| **直前の検索結果を保持する（Property 30）** | **OK**（30 件 / 一意 SKU 30 件が残存） |
| 入力欄にエラーを表示する | OK |
| 表示が「許容範囲 1〜100 の整数」を示す | **要件の文面とは異なる** |

**表示されたのはブラウザ標準のバリデーションメッセージ**（「有効な値を入力してください。有効値として最も近いのは 1 と 2 です。」）であり、要件 11.6 が求める「許容範囲（1〜100 の整数）を示すエラー」の文面ではない。`<input type="number">` の `step` 違反をブラウザが先に止めるため、アプリ側のエラー経路に到達していないと考えられる。

**観測される振る舞い（検索を実行しない・直前の結果を保持する）は要件を満たしているため欠陥ではない。** ただし「アプリが定めた文面ではなくブラウザ標準の文面が出る」という差異として記録する。`0` や `101` は `min` / `max` 違反となるため範囲が伝わる文面になると見込まれるが、**実測していない。**
...
- **この特性は実測で定量化できた。** 35.0 分に集中した recall 測定は 360 検索で 0.9667 OCU-hour（1 検索あたり約 0.00064 USD）だったのに対し、数時間に散発した UI 実測は約 25 検索で約 2.93 OCU-hour（約 0.028 USD）を消費した。**同じ 1 検索が散発すると約 44 倍高くつく**（推定。検索回数は概算）。**OpenSearch のコストは検索回数ではなく検索の時間的な密度で決まる**（第 16.4 節）
...
**さらに本検証で 1 つ追加の害が判明した。** 3 行複製は TopK を希釈するだけでなく、**近似探索の recall そのものを劣化させる。** OpenSearch の ja / K=33 が閾値を割った原因がこれである（第 4.2 節）。同一ベクトルの 3 兄弟をすべて回収しきれず、99 行の窓に 42 SKU を詰めてしまい、本来上位の SKU を押し出した。実装の欠陥ではなく、近似探索と重複ベクトルの相互作用である。

**`返却行数 ÷ 一意 SKU 件数` が回収完全性の直接指標になる。** 3 行複製の構成では 3.00 が理想値で、そこから離れた度合いが劣化の大きさである。この指標は距離やスコアとは独立した情報を与え、Ground_Truth を持たない本番環境でも計算できる。

**そして回収漏れは両バックエンドで起こる。** ブラウザ UI からの 9 クエリの実測では、DynamoDB が 1.90〜3.00、OpenSearch が 2.36〜3.00 の範囲で変動し、**どちらが劣るかはクエリ依存である**（第 4.2 節の表）。傾向として DynamoDB は 1 位距離 0.72 付近まで 3.00 を保ち、OpenSearch は距離に関係なく崩れる。**「DynamoDB は完全、OpenSearch は近似」という読み方は誤りである。**
...
### 知見 11: 検索結果が信用できないことの検知は、距離だけでは組めない

recall・機能制約・コストは測定できるが、**運用時に「この検索結果は信用できない」と判定する手段**は本検証の要件に含まれていなかった。ブラウザ UI からの実測でこの穴が見えたので、知見として記録する。

**まず、1 位の正規化距離を単独の閾値にすると誤判定する。** 次の 2 例が決定的である（いずれも日本語 / TopK 99 / 全倉庫、実測）。

| クエリ | 1 位距離 | 上位の内容 | 判定 |
|---|---|---|---|
| `1kg の業務用の大容量焙煎豆` | **0.6915** | 資材 ラベルが順位 1〜9 | **汚染** |
| `カフェインレスの豆` | **0.7103** | ケニア 生豆のみ | **健全** |

**より遠いほうが健全である。** したがって距離に閾値を置くと、0.69 で切れば `カフェインレス` を誤検知し、0.72 で切れば `1kg の業務用` を見逃す。どちらに置いても外れる。

**代わりに 2 つの指標が使える。**

| 指標 | 算出 | 検知できるもの |
|---|---|---|
| **カテゴリ整合性** | 返却行の `category` を見て、クエリの意図と異なるカテゴリが上位に混じっていないかを判定する | 資材の混入。距離を見る必要がなく、上の 2 例を正しく区別できる |
| **兄弟の完全性** | `返却行数 ÷ 一意 SKU 件数` が理想値（本構成では 3.00）から離れた度合い | 近似探索の劣化。距離とは独立した情報を与える（知見 3） |

距離は**補助指標としては有効**である。DynamoDB の回収漏れは 1 位距離 0.72 付近から始まる（第 4.2 節）。ただし単独の判定条件にはできない。

**本検証で観測した劣化は 3 種類あり、互いに独立している。**

1. **兄弟の回収漏れ**（`返却行数 ÷ 一意 SKU` が 3.00 から離れる）。両バックエンドで起こり、どちらが劣るかはクエリ依存
2. **資材の混入**。クエリの中で強く一致する語がサイズ表現しかないときに起こる（第 9.4 節）
3. **カテゴリ内のドリフト**。距離が遠いとき同カテゴリの別の性質へ流れる（`アイスコーヒー向けの深煎り豆` に生豆が混じる、`カフェインレスの豆` が生豆を返す）

これらは同時に起こりうるが相関しない。`ハンドドリップに向いた焙煎豆` は距離 0.5195 と近く資材も出ないのに OpenSearch の回収率が 2.36 で最も悪く、`アイスコーヒー向けの深煎り豆` は距離 0.7566 と遠いのに OpenSearch は 2.91 で良好である。**単一の指標で品質を代表させることはできない。**

**要件への含意。** 本検証の要件 13 は recall を、要件 15 は機能制約を、要件 14 はコストを測ることを求めているが、**「結果の信頼性を運用時に判定する手段」を求める要件がない。** ベクトル検索を本番に載せる場合、この判定は Ground_Truth を持たない環境で行う必要があるため、recall とは別の指標を設計時に決めておく必要がある。
...
| ブラウザ上での UI 操作 | 13.18 の時点では実行環境に手段が無く、共有関数と `capabilities` 応答の実測および jsdom のコンポーネントテスト 17 件で代替した。**その後ブラウザから 25 回程度の追加観測を実施し**（第 1.1〜1.7 節 / 第 4.2 節 / 第 9.4 節 / 第 9.6 節 / 第 9.7 節 / 知見 11 の根拠）、`docs/measurements/ui-investigation-2026-08-22.json` に記録した |
| **倉庫フィルタ有効時の Recall_At_K（要件 13.14）** | フィルタ有効時の Ground_Truth 再計算を伴う recall 測定は実施していない。**フィルタ自体の挙動は第 9.6 節で実測済み**だが、recall の数値は全倉庫条件のものだけである |
| 入力検証の `0` / `101` / 空クエリ / 全角スペースのみ | 実機では `1.5` のみ確認した（第 9.7 節）。他はコンポーネントテストのみ |
| レイテンシ最適化推論の失敗往復が占める時間 | `embeddingLatencyMs` に含まれるが内訳を分離していない（第 5.1 節）。Bedrock を直接 2 回叩けば測れる |
...
**2026-08-22 に部分撤収を実施した（案 A）。** 課金しうるリソースはすべて削除し、**Vector_Table・Query_Vector_Cache・Collection Group・埋め込みバッチ Lambda は意図的に残した。**

**採った手順は第 16.2 節の全 6 手順ではなく、手順 1 のみである。**
...
段階ゲートを外して再合成すると Stage A に戻り、Collection / Index / DynamoDB ベクトルインデックス 2 本 / 検索 Lambda 4 本 / API ルート 4 本 / データアクセスポリシーが削除される。**Vector_Table・Query_Vector_Cache・Collection Group は無条件に作られるため、削除には `backend.ts` と `dynamodb-tables.ts` の変更が必要であり、本撤収では行っていない。**

#### 案 A を選んだ理由

本リポジトリは複数の検証テーマを載せた再利用可能なテストベッドであり、README がベクトル検索の有効化手順を前提に書かれている。Vector_Table を残すことで次の利得がある。

- **DynamoDB 側の再検証コストが 18 分に下がる。** ベクトルは Vector_Table に残っているため、フラグを立て直せばインデックスがバックフィルで再構築される。**Bedrock 呼び出しは 0 回**である（完全撤収した場合は複製 → 埋め込み 95 分 → インデックス 18 分の約 2 時間が毎回必要）
- 残存分の費用は **Vector_Table のストレージ約 0.07 USD/月のみ**（推定）。Collection を含まない Collection Group が 0 課金であることは段階 4 の 1 時間観測と第 8.2 節の約 24.3 時間の実測で確認済み

**OpenSearch 側は Collection ごと削除したため再投入が必要である。** 再検証時に `phase = "embed"` を `forceRegenerate: true` で実行することになる（要件 4.5 のスキップ判定が Vector_Table の状態を見るため、強制指定なしでは OpenSearch への書き込みも行われない）。

#### 確認結果（`npm run vector:measure -- --teardown-check` / 2026-08-22T14:09:58Z）

| 確認項目 | 結果 | 備考 |
|---|---|---|
| `ListCollections` に `kiro-inventory-vector` が無い | **OK** | 残るのは既存の `kiro-inventory-search` のみ |
| **Good_Table が段階 0 のスナップショットと同一（要件 1.5）** | **OK** | **10 件の抽出アイテムを含め相違なし** |
| OSIS `kiro-inventory-pipeline` が `STOPPED` のまま（要件 6.9 / 6.10） | **OK** | `LastUpdatedAt` は 2026-08-11T22:47:56+09:00 で本検証の開始前 |
| `ListTables` に Vector_Table と Query_Vector_Cache が無い | **NG（意図的）** | 案 A のため残置 |
| `ListCollectionGroups` に `kiro-inventory-vector-group` が無い | **NG（意図的）** | 同上 |
| `SearchOCU` / `IndexingOCU` が 0 | **NG（窓の問題）** | 直近 24 時間の最大 2.0000 OCU。ただしこれは同日 11:10〜11:50Z に実施した UI 実測を含む窓であり、**撤収後の現在値ではない**（下記） |

**OCU の判定は集計窓の性質による見かけの NG である。** 直近 3 時間（11:10〜14:15Z）で切り直すと `IndexingOCU` は最小・平均・最大すべて 0、`SearchOCU` の 2.0000 は 11:10〜11:50Z の検索区間に由来し、**その後 60 分以上の連続 0 OCU 区間が成立している**（0 OCU 区間 2 件 / 合計 110.0 分 / 最長 65.0 分。実測）。`--teardown-check` の既定窓が直近 24 時間のローリングウィンドウであることは知見 9 に記録した性質と同じである。

#### DynamoDB ベクトルインデックスの削除確認には CLI を使えない

**`aws dynamodb describe-table` は `VectorIndexes` を `null` で返すが、これは削除の証拠にならない。** ローカルの AWS CLI 2.35.9 は当該フィールドを解釈できず、Stage B で 2 本存在した時点でも `null` を返していた（知見 10、第 15 節）。

そのため**リポジトリ同梱の `@aws-sdk/client-dynamodb` 3.1112.0**（当該フィールドを含む境界 3.1103.0 以降）で `DescribeTable` を呼んで確認した。

| 対象 | `VectorIndexes` キー | 本数 | 備考 |
|---|---|---|---|
| `kiro-roasters-inventory-vector` | **不在** | **0** | Stage B では同一 SDK で 2 本返っていた（第 15 節）。確実に削除されている |
| `kiro-roasters-inventory-good` | 不在 | 0 | 全期間を通じて 0 本（要件 1.6） |

Vector_Table は `TableStatus: ACTIVE` / `ItemCount: 15,000` / `TableSizeBytes: 138,202,024` を保っており、**日英のベクトルは残置されている。**

#### Lambda と API ルート

| リソース | 撤収後 |
|---|---|
| Lambda（`kiro-vector*`） | **`kiro-vector-embed-batch` の 1 本のみ**。検索系 4 本（`query-embed` / `search-ddb` / `search-aoss` / `capabilities`）と Index_Provisioner は削除された |
| API Gateway `/vector-search` 配下 | **`/vector-search/embed-batch`（POST / OPTIONS）と `/vector-search`（OPTIONS）のみ**。検索系 4 ルートは削除された |

#### 累積課金（撤収時点）

**10.4167 OCU-hour × 0.24 USD = 2.50 USD**（上限 20 USD / 残り 17.50 USD）。第 8.3 節に記載した 1.80 USD から **0.70 USD 増加**しており、これは UI からの追加観測（知見 11 / 第 9.4 節の根拠となった 25 回程度の検索）に由来する。

**この増分は OpenSearch のコスト特性を定量的に示している。** 第 10.4 節に述べた「検索終了から `SearchOCU` が 0 に落ちるまで約 14 分の遅れがある」ことの帰結である。

| 実施内容 | AOSS 検索回数 | 消費 OCU-hour | 1 検索あたり |
|---|---|---|---|
| recall 測定（35.0 分に集中） | 360 | 0.9667 | **約 0.00064 USD** |
| UI からの追加観測（数時間に散発） | 約 25 | 約 2.93 | **約 0.028 USD** |

**同じ 1 検索が、散発すると集中時の約 44 倍高くつく**（推定。検索回数は概算）。散発的なクエリは 1 回ごとに約 14 分ぶんの OCU を引き連れるためである。**OpenSearch のコストは検索回数ではなく検索の時間的な密度で決まる。** 低頻度の社内ツールでは、この特性が第 1.3 節の Cold_Start 18〜19 秒と合わせて DynamoDB 側に有利に働く。
...
| `ui-investigation-2026-08-22.json` | **ブラウザ UI からの追加観測（本改訂の根拠）。**日英 3 組の対比、TopK 99 での 9 クエリの兄弟回収率、資材混入とサイズ語の関係、レイテンシの条件依存、および本ドキュメントの 3 箇所の訂正内容 |
